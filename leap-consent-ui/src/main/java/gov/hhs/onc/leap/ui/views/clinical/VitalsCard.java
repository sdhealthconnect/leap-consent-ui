...
public class VitalsCard extends VerticalLayout {
    ...
    public VitalsCard(FHIRObservation fhirObservation) {
    ...
    private Component createObservation(Observation observation) {
        ...
        String diastolic = "";
        String sysstolic = "";
        if (observationName.equals("Blood Pressure")) {
            try {
                diastolic = observation.getComponent().get(0).getValueQuantity().getValue().toString();
                sysstolic = observation.getComponent().get(1).getValueQuantity().getValue().toString();
                qtyValue = sysstolic + "/" + diastolic;
                qtyUnit = "mm[Hg]";
    ...
    private Collection<Observation> getAllPatientObservations() {
        ...
        Collection<Observation> collection = fhirObservation.getPatientObservationsByType(consentSession.getFhirPatientId(), "vital-signs");
        return collection;
    }

    private void init() {
        setGrid();
        coreLayout = new FlexBoxLayout(createHeader("Vitals"), grid);
